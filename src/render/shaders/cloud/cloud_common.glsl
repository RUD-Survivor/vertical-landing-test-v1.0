...
float cloudMap(vec3 posMeter,float normalizedHeight)//单位是米，根据位置和归一化高度计算云密度
{
    ...
    vec2 sampleUv=posKm.xz*frameData.sky.atmosphereConfig.cloudWeatherUVScale;//实际使用的时候，这一行得改成行星尺度经纬度/球面UV坐标采样，
    ...
    (frameData.appTime.x*cloudSpeed*50+posMeter.xz)*0.000001+0.5
    ).x;
    //用curl噪声2D图+世界xz（米）+ 时间动画做UV。RocketSim3D 是行星+浮点原点，直接搬posKm.xz做weather，在轨道上/全球尺度会云图随相机乱飘。应保留cloud.frag的geographic weather
...
float fogNoise)
{
    float sceneZ=textureLod(sampler2D(inHiz,pointClampEdgeSampler),uv,3).r;//用层次化深度（Hi-Z）查当前像素处场景有多近，用来限制云 ray march 不要画到 建筑物/地形/火箭 前面
    //sceneZ 读出来得场景深度值；inHiz Hi-Z纹理，每像素存储该方向上得最近场景深度的mip链
    //uv 当前屏幕像素uv;pointClampEdgeSampler点采样 + 边缘 clamp（不插值、不 repeat）
    //textureLod(...,3)固定采第3级mip，偏保守
    //.r深度存在R通道
    ...
    //加上行星半径的Y轴偏移，所以worldPos是大气空间中、以地心为原点的相机位置
    ...
    //从 天空辐照度 cubemap 取 向上 (0,1,0) 方向的环境光，用于后面 groundLit（云底、地面反射贡献）
    //依赖 inSkyIrradiance（binding 27）,当前工程没有
    ...
    if(!bEarlyOutCloud)//主云ray march
    {
        ...
        // upScaleColor：天空辐照度 cubemap 在「正上方」(0,1,0) 的 RGB，近似云底/云内收到的半球环境光强度。
        // 物理：地面与低空大气把天光散射回云底；cube 预积分了半球 incoming radiance。
        // RocketSim3D：无 inSkyIrradiance；cloud.frag 用 skyAmbColor + atmTransAtCloud 实时估算，更简单且与 atmo 一致。
        ...
        // 物理模型：参与介质 RTE 的前向离散形式；每步把局部散射光叠到 scatteredLight，并衰减 transmittance。
        {
            ...
            if(stepCloudDensity>0.0)
            {
                ...
                // 不是精确物理（球面上真正的 up 应是径向 normalize(pos-planetCenter)）；
                // RocketSim3D 应改径向 up，不能用固定 (0,1,0)。
                //
                // 【这个变量代表什么】
                ...
                {
                    participatingMediaAmbient=volumetricShadow(samplePos,vec3(0,1,0),atmosphere,-1,SkyMsExtinction);
                    // 天空方向用 SkyMsExtinction(0.5)系数衰减高阶，比太阳路径更「糊」。
                    // RocketSim3D：vec3(0,1,0) 是 Y-up 假设；应改为 normalize(samplePos - planetCenter)。
                ...
                *saturate(1.0-GroundOcc+normalizedHeight)*frameData.sky.atmosphereConfig.cloudFogFade;
                ...
                // RocketSim3D：无 skyBackgroundColor LUT / inSkyIrradiance 时，可仿 cloud.frag 的 groundLit+skyLit 简化式。

                vec3 ambientLit=upScaleColor*powderEffect*(1.0-sunDirection.y)
                *atmosphereTransmittance;
                // ambientLit：云内环境光（天光经云自阴影后的散射照明）。
                // (1-sunDirection.y)：太阳越低（y 小），环境天光占比越大；正午直射主导时减弱。
                // 乘 powderEffect：环境路也带一点粉末感。
                // RocketSim3D：sunDirection.y 在地心球坐标下应改为 dot(sunDir, localUp)。
        ...
        // rayHitPosWeight>0 表示本像素 march 中至少有过非零云密度且累加了权重；全无云则跳过。
        {
            ...
            // 大气空间 → 引擎渲染空间（flower 的 camWorldPos 坐标系）。
            // 减 bottomRadius Y 偏移：与 316 行 worldPos 构造互逆。
            // RocketSim3D：改为相对 pc.planetCenter + frame.viewPos 单位体系。
            ...
            // 空气透视：相机与云之间的大气散射，使远处云发雾、发蓝、对比度降（观察路径上的雾，非太阳光路径）。
            {
                ...
                // RocketSim3D：等价效果由 cloud 与 atmo 合成 pass 完成，不必单独 Froxel。
            }
        }
    }
    //===================================================
    ...
    // RocketSim3D：无单独 God Ray pass；低空雾/atmo 已在 atmo.frag，通常整段不移植。
    {
        ...
        {
            vec3 P0=convertToAtmosphereUnit(rayPosWP,frameData)+vec3(0.0,atmosphere.bottomRadius,0.0);
            // P0：当前雾采样点转到大气空间 (km)，与主云 march 的 worldPos/samplePos 同一套坐标。
