...
void main() {
    ...
    vec3 texCoord = sph * 5.0;

    float warp = fbm(texCoord + vec3(0.0, texCoord.y * 3.0, 0.0));
    float bands = sin(texCoord.y * 15.0 + warp * 5.0);
    bands = smoothstep(-1.0, 1.0, bands);

    vec3 color1 = pc.baseColor.rgb;
    vec3 color2 = pc.baseColor.rgb * 0.4 + vec3(0.3);
    ...
    rim = pow(rim, 3.0);
    vec3 atmosColor = pc.baseColor.rgb * rim * 0.5;
