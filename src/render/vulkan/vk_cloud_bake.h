...
//   cloud_curlnoise.comp   → 256² R8  (Phase 2: 局部碎片湍流细节，2D)
// ==========================================================================
...
struct VkCloudBake {
    ...
    VkPipeline            curlPipeline    = VK_NULL_HANDLE;
    ...
    bool init(VulkanContext& ctx) {
        ...
        if (!createComputePipeline(ctx, "src/render/shaders/spirv/cloud_basicnoise.comp.spv", basicPipeline) ||
            ...
            !createComputePipeline(ctx, "src/render/shaders/spirv/cloud_curlnoise.comp.spv", curlPipeline)) {
            return false;
        }
    ...
    bool bakeCurlNoise(VulkanContext& ctx, VkTexture2D& tex) {
        return dispatchBake2D(ctx, tex, curlPipeline, 256, 256);
    }
    ...
    void shutdown(VkDevice device) {
        ...
        if (curlPipeline   != VK_NULL_HANDLE) { vkDestroyPipeline(device, curlPipeline,   nullptr); curlPipeline   = VK_NULL_HANDLE; }
...
private:
    ...
    void updateStorageBinding2D(VulkanContext& ctx, VkTexture2D& tex) {
        VkDescriptorImageInfo imgInfo{};
        imgInfo.imageView   = tex.view;
        imgInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    ...
    // 与 dispatchBake 同一套流程，只是 2D 图像、gz 恒为 1（复用同一个 set0/binding0 布局，
    // STORAGE_IMAGE 的 layout 不区分 2D/3D，见 cloud_curlnoise.glsl 的 image2D 声明）。
    bool dispatchBake2D(VulkanContext& ctx, VkTexture2D& tex, VkPipeline pipeline,
                      uint32_t w, uint32_t h) {
        if (pipeline == VK_NULL_HANDLE || tex.image == VK_NULL_HANDLE) return false;

        updateStorageBinding2D(ctx, tex);
