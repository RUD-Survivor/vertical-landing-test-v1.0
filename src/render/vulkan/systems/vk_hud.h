...
struct HudVertex { float x,y,u,v, r,g,b,a; };  // 12 floats = 48 bytes
...
struct VkHUDSystem {
    ...
    bool init(VulkanContext& ctx, VkFormat swapFmt) {
        ...
        // 管线创建（lambda 避免 goto 跳过变量初始化的 C2362 问题）
        bool pipeOk = [&]() -> bool {
            ...
            if(vkCreateDescriptorSetLayout(ctx.device,&sl,nullptr,&setLayout)!=VK_SUCCESS)return false;
            VkPipelineLayoutCreateInfo pl{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,nullptr,0,1,&setLayout,0,nullptr};
            ...
            VkPipelineVertexInputStateCreateInfo vi{VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO,nullptr,0,1,&vb,3,va};
            ...
            return vkCreateGraphicsPipelines(ctx.device,VK_NULL_HANDLE,1,&pci,nullptr,&pipeline)==VK_SUCCESS;
        }();
        destroySMs();
        if(!pipeOk)return false;

        VkBufferCreateInfo bci{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
        bci.size=MAX_VERTS*sizeof(HudVertex); bci.usage=VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        VmaAllocationCreateInfo aci{};
        aci.usage=VMA_MEMORY_USAGE_CPU_TO_GPU; aci.flags=VMA_ALLOCATION_CREATE_MAPPED_BIT;
        ...
        VkDescriptorSetAllocateInfo ai2{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,nullptr,descPool,1,&setLayout};
    ...
    void pushQuad(float x, float y, float w, float h, float r=1,float g=1,float b=1,float a=1){
        ...
        HudVertex v[6]={
            {x,y,0,0, r,g,b,a},{x+w,y,1,0, r,g,b,a},{x+w,y+h,1,1, r,g,b,a},
