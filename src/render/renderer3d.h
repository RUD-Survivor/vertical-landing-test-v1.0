...
public:
  ...
  Renderer3D() {
    ...
    const char* atmoFragSrc = R"(
      ...
      // Cloud tuning uniforms — must be declared before setupPlanetProfile() uses them
      ...
      uniform sampler2D uWeatherMap;  // 2048x1024: R=coverage_bias, G=cloud_type, B=humidity, A=land_mask
      ...
      // Worley (cellular) noise — 3×3×3 neighbor search, returns nearest feature distance [0,1]
      ...
      // Perlin-Worley: Worley carves rounded cellular blobs into Perlin base → cauliflower cumulus shape
      float fbm_pw(vec3 p, int octaves) {
          float pn = fbm(p, octaves);
          float wn = 1.0 - worley3d(p * 1.8); // inverted: bright cell interior, dark edges
          return clamp(pn - (1.0 - wn) * 0.32, 0.0, 1.0);
      }
      // Curl noise on unit sphere: ∇×(φ·n̂) → divergence-free tangential wind velocity.
      // Finite-difference gradient of noise3d projected to tangent plane, then rotated 90°.
      // 6 noise evaluations per call — use sparingly (coverage map only, not per-step detail).
      vec3 curlSph(vec3 p, float scale, float toff) {
          ...
          grad -= p * dot(p, grad); // project onto tangent plane (remove radial component)
          return cross(p, grad);    // 90° rotation on tangent plane = curl on sphere
      }
      // Returns raw Earth coverage [0,1] BEFORE smoothstep — used for F3 debug visualization.
      // Must mirror the coverage computation in cloudDensity() Layer 1 exactly.
      float rawCoverage(vec3 sph, float t) {
          ...
          vec3 va = cross(cs, ax);
          // Curl noise: 4-sample finite-difference gradient of φ, rotated 90°.
          // Divergence-free → flow lines are closed vortex rings.
          const float EPS = 0.06;
          vec3 slLo = vec3(t * 0.003, 0.0, t * 0.002);
          // scale 1.5 → vortex diameter ~4000km; at 0.6 was ~8000km (one vortex = half the planet)
          ...
          // scale 2.5/1.0 → lo displacement ≈0.9, hi ≈0.72
      ...
      float cloudDensity(vec3 p, float h, bool highDetail) {
          ...
          if (uPlanetIdx == 3) {
              ...
              if (h >= gCloudMinAlt && h <= 7.0) {
                  ...
                  // latWarp coefficient reduced from 0.8 → 0.3 to break zonal symmetry:
                  // high latWarp makes clouds at same latitude look identical → horizontal bands.
                  float latWarp = sph.z * sph.z * sph.z * 0.3;
                  ...
                  // Linear falloff from edge → stronger gradient → more visible shear bands
                  float cycInfluence = smoothstep(cycRadius, 0.0, cycDist);
                  if (cycInfluence > 0.001) {
                      // ~103° max twist: FBM bands shear into visible spiral arms.
                      // Old 459° scrambled noise into a featureless ring — spiral structure lost.
                  ...
                  vec3 vAxis = cross(coverageSph, wAxis);  // meridional tangent
                  // Curl noise domain warp: ∇φ rotated 90° is divergence-free.
                  // Flow lines of a div-free field are closed vortex rings → spiral/comma structures
                  ...
                  // Global weather map: encodes land/sea climate distribution.
                  // R=coverage_bias (desert≈0.04, ITCZ ocean≈0.82), G=cloud_type, B=humidity.
                  // Equirectangular UV: atan→lon [-π,π]→[0,1]; asin→lat [-π/2,π/2]→[0,1].
                  // Only applied for Earth (planetIdx==3); other planets keep unity scaling.
                  vec2 wUV = vec2(
                  ...
                  // Linear ramp (not smoothstep): preserves continuous coverage gradient.
                  ...
                  if (cycInfluence > 0.001) {
                      ...
                      // Avoids forced white ring; spiral arms come from twisted FBM naturally.
                      coverage = mix(coverage, clamp(coverage * 1.9, 0.0, 1.0), cycInfluence * 0.7);
                      // Ragged eye boundary: FBM noise on radius so edge is organic, not circular
                  ...
                  // Cloud type [0=stratus flat, 0.5=cumulus, 1=cumulonimbus tall] from low-freq spatial variation
                  float cloudType = mix(texture(uCoverTex3D, (warpedSph * 2.5 + vec3(31.4, 17.8, 42.1)) * INV8).r, weather.g, 0.55);
                  // Per-type height profiles (altNorm: 0=cloud base, 1=7km top)
                  float stratusGrad      = altBase * (1.0 - smoothstep(0.15, 0.30, altNorm)); // flat, low
                  float cumulusGrad      = altBase * (1.0 - smoothstep(0.55, 0.80, altNorm)); // medium blobs
                  float cumulonimbusGrad = altBase * (1.0 - smoothstep(0.90, 1.00, altNorm)); // tall towers
                  // Lower-freq top noise (scale 8 vs old 40) → large-scale Cb vs stratus regions
                  float topNoise  = texture(uCoverTex3D, (windSph1 * 8.0 + vec3(53.2, 29.7, 71.4 + t * 0.01)) * INV8).r;
                  float topHeight = mix(0.40, 1.00, topNoise); // cloud top at 40%–100% of layer thickness
                  float topMask   = 1.0 - smoothstep(topHeight - 0.06, topHeight + 0.06, altNorm);
                  // Blend height profiles by cloud type, then cut off above per-cluster top
...
R"(
                  ...
                  // Altitude encodes as sphere-radius expansion: tc magnitude increases by h*0.45 over 4km band.
                  ...
                  if (highDetail) {
                      ...
                      // Perlin-Worley screen blend (Schneider 2015):
                      // 3-oct FBM for aperiodic macro structure; baked inverted Worley (G→1-G) at the
                      // same scale as octave-1 (warpedTc*0.125 → UV features at ~80km) carves organic
                      // concavities at cell edges → cauliflower silhouette without circular artifacts.
                      // Signed blend: (wInv-0.5)*(1-n)*0.50 → zero bias at mean wInv=0.5, ±0.10 push
                      // at extremes; preserves coverage level while adding Worley curvature.
                      n = fbm(warpedTc, 3);
                      ...
                      // Near cloud top: invert erosion direction → cauliflower turrets (cumulus/Cb tops)
                      ...
                      {
                          // Baked Worley (G channel, UV=p*S/8): hardware bilinear pre-filters high-freq →
                          // eliminates temporal aliasing from runtime worley3d() near the isosurface.
                          // Seed offsets /8 to convert from noise-space to UV-space.
                      ...
                      // cloudExists gates all LOD sculpting so blocks can only REFINE existing clouds,
                      // never CREATE new ones from empty space — prevents LOD-induced cloud pops.
                      ...
                      n = fbm(tc, 2);
                      // P-W screen blend (mirrors primary path; uses tc≈warpedTc, close enough for shadows).
                      { float wInvS = 1.0 - texture(uNoiseTex3D, tc * 0.125).g;
                        n = n + (wInvS - 0.5) * (1.0 - n) * 0.50; }
                      // Shadow puff isolation: mirrors highDetail A2 (baked Worley, no runtime worley3d).
                      {
                  ...
                  // coverage=0 → density=0 (zero scatter guaranteed); coverage=0.5 → half-density cloud.
                  // min/max guards: smoothstep(e0,e1,x) is undefined when e0≥e1 (GLSL spec).
              ...
              if (h >= 8.0 && h <= gCloudMaxAlt) {
                  ...
                  float angle2 = t * -0.5;
                  // Coriolis Warping for high cirrus — reduced to 0.15 to prevent zonal banding
                  float latWarp2 = sph.z * sph.z * sph.z * 0.15; // Z is the polar axis
                  ...
                  vec3 tc = windSph2 * 50.0; // ~127km cirrus streak features

                  // Cirrus shape
                  float n = 0.0;
                  if (highDetail) {
                      float warp = fbm(tc * 0.4, 2); // coarser warp
                      n = fbm(tc + vec3(warp * 0.7, warp * 0.9, 0.0) + vec3(t * 0.04), 4); // elongated streaks
                      // Cirrus Erosion: wispy fractal detail
                      float detail = fbm(tc * 2.5 + vec3(t * 0.08), 2);
                      n = n - detail * 0.18;
                  } else {
                  ...
                  float mask = smoothstep(0.42 + altNorm * 0.10, 0.82, n) * altShape;
                  density = max(density, mask * 0.30); // Thinner density
              }
...
R"(
          ...
          if (hitOuter) {
              ...
              if (cloudSegLen > 0.0) {
                  ...
                  // Anti-boiling: inside/near cloud layer, halve bigStep and reduce jitter amplitude.
                  // Root cause: IGN jitter shifts first-cloud-hit by up to 1 fineStep; bigStep=4x
                  // causes the probe-based skip to jump over the entry region discontinuously → per-frame
                  // first-hit variance at steep isosurface = violent boiling at close range.
                  ...
                  for (int ci = 0; ci < MAX_CLOUD_ITER; ci++) {
                      ...
                      // Probe at density-correlated scale (UV≈10, same as P-W blend in cloudDensity).
                      // Old probeSph*0.125 (UV≈0.125) sampled a completely different noise region
                      // than the density warpedTc*0.125 (UV≈10): probe decisions were uncorrelated
                      // with actual cloud presence → clouds popped with any camera movement.
                      // Fix: probeSph*10.0 matches the density UV, making probe threshold meaningful.
                      // Time offset mirrors density's tc animation so probe tracks cloud drift.
