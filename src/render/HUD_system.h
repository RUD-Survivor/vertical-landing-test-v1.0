    ...
    if (cam.mode == 2 && !SOLAR_SYSTEM.empty()) {
        ...
        if (transfer_window_menu && flight_assist_menu) {
            ...
            if (hover_tgt_l && hlmb && !hlmb_prev) {
                ...
                do { transfer_target_body--; if (transfer_target_body < 1) transfer_target_body = (int)SOLAR_SYSTEM.size() - 1; } while (transfer_target_body == origin || transfer_target_body == 4 || transfer_target_body == 0);
            ...
            if (hover_tgt_r && hlmb && !hlmb_prev) {
                int origin = TransferCalculator::getTransferOriginBody();
                do { transfer_target_body++; if (transfer_target_body >= (int)SOLAR_SYSTEM.size()) transfer_target_body = 1; } while (transfer_target_body == origin || transfer_target_body == 4 || transfer_target_body == 0);
                transfer_result_valid = false;
            }
