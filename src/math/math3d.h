...
struct Vec2 {
  ...
  Vec2 operator/(double s) const { double inv = 1.0 / s; return Vec2(x * inv, y * inv); }
...
struct Vec3 {
  ...
  Vec3 operator/(double s) const { double inv = 1.0 / s; return Vec3(x * inv, y * inv, z * inv); }
...
struct Quat {
  ...
  static Quat fromEuler(double pitch, double yaw, double roll) {
    Quat qx = fromAxisAngle(Vec3(1.0, 0.0, 0.0), pitch);
    Quat qy = fromAxisAngle(Vec3(0.0, 1.0, 0.0), yaw);
    Quat qz = fromAxisAngle(Vec3(0.0, 0.0, 1.0), roll);
    return qy * qx * qz;
  }
  ...
  Quat normalized() const {
    ...
    if (len < 1e-12) return Quat(1, 0, 0, 0);
    double inv = 1.0 / len;
  ...
  Vec3 forward() const { return rotate(Vec3(0.0, 1.0, 0.0)); }
  Vec3 right()   const { return rotate(Vec3(1.0, 0.0, 0.0)); }
  Vec3 up()      const { return rotate(Vec3(0.0, 0.0, 1.0)); }
  ...
  static Quat slerp(const Quat& a, const Quat& b, double t) {
    ...
    double wa = std::sin((1.0 - t) * theta) / sin_t;
    double wb = std::sin(t * theta) / sin_t;
    return Quat(
      wa * a.w + wb * b2.w,
      wa * a.x + wb * b2.x,
      wa * a.y + wb * b2.y,
...
struct Mat4 {
  ...
  Vec3 transformPoint(const Vec3& v) const {
    double rx = m[0]*v.x + m[4]*v.y + m[8]*v.z  + m[12];
    double ry = m[1]*v.x + m[5]*v.y + m[9]*v.z  + m[13];
    double rz = m[2]*v.x + m[6]*v.y + m[10]*v.z + m[14];
    double rw = m[3]*v.x + m[7]*v.y + m[11]*v.z + m[15];
    if (std::abs(rw) > 1e-12) { rx /= rw; ry /= rw; rz /= rw; }
  ...
  static Mat4 perspective(double fovY_rad, double aspect, double zNear, double zFar) {
    Mat4 r;
    for (int i = 0; i < 16; i++) r.m[i] = 0;
    double f = 1.0 / std::tan(fovY_rad * 0.5);
    r.m[0]  = f / aspect;
    r.m[5]  = f;
